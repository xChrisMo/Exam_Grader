...
class TestMappingService(unittest.TestCase):
    ...
    def test_extract_questions_and_answers(self):
        ...
        self.assertGreater(
            len(guide_items), 0, "Should extract at least one item from marking guide"
        )

        # Test extraction from student submission
        submission_items = self.mapping_service.extract_questions_and_answers(
            self.student_submission
        )
        self.assertGreater(
            len(submission_items),
            0,
            "Should extract at least one item from student submission",
        )
    ...
    def test_map_submission_to_guide(self, mock_llm_service_class):
        ...
        result, error = mapping_service.map_submission_to_guide(
            self.marking_guide, self.student_submission
        )
        ...
        if error:
            self.assertIn(
                "LLM service is required",
                error,
                "Should indicate LLM service requirement",
            )
        else:
            # If no error, check that mapping was successful
            self.assertEqual(
                result.get("status"), "success", "Mapping status should be success"
            )

            # Check that mappings were created
            mappings = result.get("mappings", [])
            self.assertGreaterEqual(
                len(mappings), 0, "Should create mappings or empty list"
            )
            ...
            for mapping in mappings:
                ...
                self.assertIn(
                    "submission_id", mapping, "Mapping should have submission_id"
                )
    ...
    def test_empty_content(self):
        ...
        guide_items = self.mapping_service.extract_questions_and_answers("")
        self.assertEqual(
            len(guide_items), 0, "Should return empty list for empty content"
        )
    ...
    def test_determine_guide_type_questions(self, mock_openai):
        ...
        guide_type, confidence = mapping_service.determine_guide_type(
            guide_with_questions
        )

        # Verify results
        self.assertEqual(
            guide_type, "questions", "Should identify guide as containing questions"
        )
        self.assertGreater(
            confidence, 0.5, "Should have high confidence in determination"
        )
    ...
    def test_determine_guide_type_answers(self, mock_openai):
        ...
        # Verify results
        self.assertEqual(
            guide_type, "answers", "Should identify guide as containing answers"
        )
        self.assertGreater(
            confidence, 0.5, "Should have high confidence in determination"
        )
    ...
    def test_llm_based_mapping_questions(self, mock_openai):
        ...
        # Test mapping
        result, error = mapping_service.map_submission_to_guide(
            "Sample marking guide content", "Sample student submission content"
        )
        ...
        self.assertGreaterEqual(
            len(result.get("mappings", [])), 1, "Should create at least one mapping"
        )
        ...
        self.assertIn(
            "guide_type",
            result.get("metadata", {}),
            "Metadata should include guide_type",
        )
