...
def get_progress(session_id):
    ...
    try:
        ...
        try:
            ...
            # Safely handle all fields to prevent JSON serialization issues
            progress = {
                'percentage': float(progress_info.progress_percentage) if progress_info.progress_percentage is not None else 0.0,
                'current_step': str(progress_info.current_step) if progress_info.current_step else 'Unknown',
                'status': str(progress_info.status) if progress_info.status else 'unknown',
                'guides_processed': int(progress_info.guides_processed) if progress_info.guides_processed is not None else 0,
                'questions_extracted': int(progress_info.questions_extracted) if progress_info.questions_extracted is not None else 0,
                'average_confidence': float(progress_info.average_confidence) if progress_info.average_confidence is not None else 0.0,
                'error_message': str(progress_info.error_message) if progress_info.error_message else None
            }

            # Clean any problematic characters from string fields
            for key, value in progress.items():
                if isinstance(value, str):
                    # Remove or replace problematic characters that could cause JSON parsing issues
                    progress[key] = value.replace('\\', '\\\\').replace('"', '\\"').replace('\n', '\\n').replace('\r', '\\r')
...
def view_progress(session_id):
    ...
    try:
        ...
        # Convert to dict for template with safe string handling
        def safe_string(value):
            """Safely convert value to string and escape problematic characters"""
            if value is None:
                return None
            str_value = str(value)
            # Escape characters that could cause JSON/template issues
            return str_value.replace('\\', '\\\\').replace('"', '\\"').replace('\n', '\\n').replace('\r', '\\r')
        
        session = {
            'id': session_obj.id,
            'name': safe_string(session_obj.name),
            'status': safe_string(session_obj.status),
            'created_at': session_obj.created_at.isoformat() if session_obj.created_at else None,
            ...
            'avg_confidence': float(session_obj.average_confidence) if session_obj.average_confidence is not None else 0.0,
            'progress_percentage': float(session_obj.progress_percentage) if session_obj.progress_percentage is not None else 0.0,
            'current_step': safe_string(session_obj.current_step) or 'Unknown',
            'error_message': safe_string(session_obj.error_message)
        }
