...
def dashboard():
    ...
    try:
        ...
        recent_results = GradingResult.query.order_by(
            GradingResult.created_at.desc()
        ).limit(10).all()
...
def logs():
    ...
    try:
        ...
        log_files = [f.name for f in Path('logs').glob('*.log')] if Path('logs').exists() else []
    ...
    try:
        ...
        if hasattr(core_service, '_processing_cache'):
            ...
            if hasattr(cache, 'info'):
                ...
                stats.update({
                    'cache_entries': cache_info.currsize if hasattr(cache_info, 'currsize') else 0,
                    'hit_rate': f"{cache_info.hits / (cache_info.hits + cache_info.misses) * 100:.1f}%" if hasattr(cache_info, 'hits') and (cache_info.hits + cache_info.misses) > 0 else 'N/A'
                })
