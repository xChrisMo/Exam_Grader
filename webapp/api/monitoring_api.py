...
def get_system_health():
    ...
    try:
        ...
        if not health_status:
            return (
                jsonify({"status": "error", "message": "Health monitor not available"}),
                503,
            )
...
def get_service_health(service_name: str):
    ...
    try:
        ...
        if not health_status or "services" not in health_status:
            return (
                jsonify({"status": "error", "message": "Health data not available"}),
                503,
            )
        ...
        return jsonify(
            {
                "status": "success",
                "data": {"service_name": service_name, "health": service_health},
                "timestamp": datetime.now(timezone.utc).isoformat(),
            }
        )
...
def get_metric(metric_name: str):
    ...
    try:
        ...
        if not metric:
            return (
                jsonify(
                    {"status": "error", "message": f"Metric {metric_name} not found"}
                ),
                404,
            )
        ...
        if include_history:
            response_data["history"] = realtime_metrics_collector.get_metric_history(
                metric_name, minutes
            )
...
def get_performance_summary():
    ...
    try:
        ...
        if not summary:
            return (
                jsonify(
                    {"status": "error", "message": "Performance data not available"}
                ),
                503,
            )
    ...
    try:
        ...
        if severity:
            try:
                ...
                return (
                    jsonify(
                        {"status": "error", "message": f"Invalid severity: {severity}"}
                    ),
                    400,
                )
        ...
        alerts = monitoring_dashboard_service.get_alerts(
            severity=severity_enum, resolved=resolved_bool, limit=limit
        )
    ...
    try:
        ...
        system_status = {
            ...
            "uptime": {
                ...
                "metrics_collection_active": dashboard_data.get("collection_interval")
                is not None,
