...
def validate_request_data(data: Dict[str, Any], required_fields: list) -> Optional[Dict[str, Any]]:
...
def process_single_submission():
    ...
    try:
        with track_operation("api_process_single", metadata={'request_id': request_id}):
            ...
            validation_errors = validate_request_data(data, ['guide_id', 'submission_id'])
...
def process_batch_submissions():
    ...
    try:
        with track_operation("api_process_batch", metadata={'request_id': request_id}):
            ...
            # Validate required fields
            validation_errors = validate_request_data(data, ['guide_id', 'submission_ids'])
            if validation_errors:
                response, status = api_error_handler.handle_validation_error(
                    validation_errors, request_id
                )
            ...
            if not isinstance(submission_ids, list) or not submission_ids:
                response, status = api_error_handler.handle_validation_error(
                    {'submission_ids': 'Must be a non-empty list'},
                    request_id
                )
                return jsonify(response), status
            ...
            for i, submission_id in enumerate(submission_ids):
                try:
                    ...
                    logger.error(f"Error processing submission {submission_id}: {sub_error}")
            ...
            batch_data = {
                'batch': {
                    ...
                    'success_rate': successful_count / len(submission_ids) if submission_ids else 0
                },
                'results': results,
                'summary': {
...
def get_submission_status(submission_id):
    ...
    try:
        with track_operation("api_submission_status", metadata={'request_id': request_id}):
            # Verify submission ownership
            submission = Submission.query.filter_by(
                id=submission_id, 
                user_id=current_user.id
            ...
            status_data = {
                ...
                'latest_result': {
                    ...
                    'percentage': (latest_result.total_score / latest_result.max_score * 100) if latest_result.max_score > 0 else 0,
