...
def system_health():
    ...
    try:
        ...
        try:
            ...
            available_deps = sum(
                1 for dep in dependency_status.values() if dep.get("available", False)
            )
        ...
        try:
            ...
            health_data["system"] = {"status": "unknown", "error": str(e)}

        # Get performance metrics
        try:
            perf_summary = performance_monitor.get_performance_summary()
            health_data["performance"] = {"status": "healthy", "metrics": perf_summary}
            ...
            health_data["performance"] = {"status": "unknown", "error": str(e)}
        ...
        response, status = api_error_handler.create_error_response(
            error=e, status_code=500, message="Health check failed"
        )
...
def service_status():
    ...
    try:
        ...
        try:
            ...
            services_data["services"]["core_service"] = {
                "status": "error",
                "error": str(e),
            }
        ...
        try:
            ...
            services_data["services"]["llm_service"] = {
                "status": "error",
                "error": str(e),
            }
        ...
        response, status = api_error_handler.create_error_response(
            error=e, status_code=500, message="Service status check failed"
        )
        return jsonify(response), status
...
def performance_metrics():
    ...
    try:
        ...
        try:
            performance_data["metrics"][
                "operations"
            ] = performance_monitor.get_all_operation_stats()
        except Exception as e:
        ...
        try:
            performance_data["metrics"][
                "errors"
            ] = processing_error_handler.get_error_statistics()
        except Exception as e:
        ...
        response, status = api_error_handler.create_error_response(
            error=e, status_code=500, message="Performance metrics retrieval failed"
        )
        return jsonify(response), status
    ...
    try:
        ...
        if task_id not in progress_store:
            response, status = api_error_handler.handle_not_found_error(
                resource=f"Processing task {task_id}", request_id=request_id
            )
            return jsonify(response), status
...
def database_status():
    ...
    try:
        ...
        try:
            ...
            db_data["connection"] = {"status": "failed", "error": str(e)}
        ...
        response = api_error_handler.create_success_response(
            data=db_data, message="Database status retrieved", request_id=request_id
        )
...
def system_alerts():
    ...
    try:
        ...
        alerts_data["alerts"].sort(key=lambda x: x.get("timestamp", ""), reverse=True)
        ...
        response = api_error_handler.create_success_response(
            data=alerts_data, message="System alerts retrieved", request_id=request_id
        )
        ...
        response, status = api_error_handler.create_error_response(
            error=e, status_code=500, message="System alerts retrieval failed"
        )
        return jsonify(response), status
