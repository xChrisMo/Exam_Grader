...
class ComprehensiveLogger:
    ...
    def _update_metrics(self, level: LogLevel, operation: Optional[str] = None):
        ...
        with self._metrics_lock:
            ...
            if operation:
                ...
                self.performance_metrics['operations'][operation]['last_seen'] = datetime.now(timezone.utc)
    ...
    def log(self, level: Union[LogLevel, str], message: str, context: Optional[LogContext] = None, **kwargs):
    ...
    def error(self, message: str, context: Optional[LogContext] = None, exception: Optional[Exception] = None, **kwargs):
        ...
        if exception:
            ...
            if self.error_tracker and ENHANCED_ERROR_HANDLING_AVAILABLE:
                try:
                    context_dict = context.to_dict() if context else self._get_current_context()
                    self.error_tracker.track_error(exception, additional_context=context_dict)
                except Exception:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        with self._metrics_lock:
            uptime = (datetime.now(timezone.utc) - self.performance_metrics['start_time']).total_seconds()
            ...
            return {
                ...
                'recent_errors': self.performance_metrics['errors'][-10:],  # Last 10 errors
...
class PerformanceLogger(ComprehensiveLogger):
    ...
    def start_operation(self, operation_name: str, context: Optional[LogContext] = None) -> str:
        ...
        self.debug(f"Started operation: {operation_name}", context, operation_id=operation_id)
        return operation_id
    
    def end_operation(self, operation_id: str, success: bool = True, result_info: Optional[Dict] = None):
        ...
        with self._timing_lock:
            if operation_id in self.operation_timings:
                ...
                context = LogContext(**operation['context']) if operation['context'] else None
                
                log_message = f"Completed operation: {operation['name']} in {duration:.3f}s"
                if result_info:
                ...
                if success:
                    ...
                    self.warning(log_message, context, 
                                operation_id=operation_id, 
                                duration=duration, 
                                success=success)
...
class SecurityLogger(ComprehensiveLogger):
    ...
    def log_authorization_failure(self, user_id: str, resource: str, action: str, ip_address: str):
        ...
        self.security(f"Authorization failure: User {user_id} attempted {action} on {resource}", context)
    
    def log_suspicious_activity(self, description: str, user_id: str = None, ip_address: str = None, **kwargs):
...
class AuditLogger(ComprehensiveLogger):
    ...
    def log_data_access(self, user_id: str, resource_type: str, resource_id: str, action: str):
        ...
        self.audit(f"Data access: {action} on {resource_type}:{resource_id} by user {user_id}", context)
    ...
    def log_data_modification(self, user_id: str, resource_type: str, resource_id: str, 
        ...
        self.audit(f"Data modified: {resource_type}:{resource_id} by user {user_id}", context)
    
    def log_system_event(self, event_type: str, description: str, user_id: str = None, **kwargs):
        """Log system event."""
        context = LogContext(
            user_id=user_id,
            additional_data={
...
def setup_logging(
    ...
    if quiet_third_party:
        third_party_loggers = [
            ...
            'werkzeug', 'urllib3', 'requests', 'PIL', 'matplotlib', 'celery'
        ]
