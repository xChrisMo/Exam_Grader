...
try:
    ...
    from .comprehensive_logger import ComprehensiveLogger, LogContext, LogLevel
...
class FlaskLoggingIntegration:
            ...
            if self.comprehensive_logger:
                ...
                self.comprehensive_logger.info(
                    ...
                    extra={
                        ...
                        "request_headers": self._sanitize_headers(
    ...
    def _after_request(self, response):
        ...
        if self.log_responses:
            client_info = self._get_client_info()
            performance_metrics = (
                self._get_performance_metrics() if self.log_performance else {}
            )
        ...
        if error.code >= 400:
            ...
            if self.structured_logger:
                log_method = (
                    ...
                    if error.code >= 500
                    else self.structured_logger.warning
                )
                log_method(
                    f"HTTP {error.code} in {request.method} {request.path}: {error.description}",
                    correlation_id=request_id,
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if logger:
                    ...
                    logger.info(
                        f"Route function completed: {func.__name__} ({duration:.3f}s)",
                        **extra_data,
                    )
                        ...
                        extra_data.update(
                            {
                                ...
                                "memory_delta_mb": end_metrics["memory_mb"]
