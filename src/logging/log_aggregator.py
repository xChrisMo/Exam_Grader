...
class LogAggregator:
    ...
    def _analyze_log_line(
        self, line: str, metrics: LogMetrics, start_time: datetime, end_time: datetime
    ):
    ...
    def _calculate_derived_metrics(self, metrics: LogMetrics):
        ...
        if metrics.total_logs > 0:
            metrics.error_rate = (
                (metrics.error_count + metrics.critical_count)
                / metrics.total_logs
                * 100
            )
...
class LogAnalytics:
    ...
    def detect_anomalies(
        self, current_metrics: LogMetrics, baseline_hours: int = 168  # 1 week
    ) -> Dict[str, Any]:
        ...
        if (
            ...
            and baseline_metrics.avg_response_time
            and baseline_metrics.avg_response_time > 0
        ):
                ...
                anomalies["detected"].append(
                    {
                        ...
                        "severity": (
                            "high" if response_time_increase > 0.8 else "medium"
                        ),
    ...
    def compare_periods(
        ...
        if metrics1.error_rate > 0:
            error_rate_change = (
                (metrics2.error_rate - metrics1.error_rate) / metrics1.error_rate
            ) * 100
        ...
        if metrics1.avg_response_time and metrics2.avg_response_time:
            response_time_change = (
                (metrics2.avg_response_time - metrics1.avg_response_time)
                / metrics1.avg_response_time
            ) * 100
        ...
        if metrics1.active_users > 0:
            user_activity_change = (
                (metrics2.active_users - metrics1.active_users) / metrics1.active_users
            ) * 100
        ...
        if metrics1.total_logs > 0:
            log_volume_change = (
                (metrics2.total_logs - metrics1.total_logs) / metrics1.total_logs
            ) * 100
