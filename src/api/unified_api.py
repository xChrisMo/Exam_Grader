...
def after_request(response):
    ...
    logger.info(
        f"API Request completed: {request.method} {request.path} - {response.status_code} ({duration:.3f}s)"
    )
    return response
            ...
            "guides": [
                {
                    ...
                    "question_count": (
                        len(guide.content.get("questions", [])) if guide.content else 0
                    ),
                }
...
def process_submission():
    ...
    try:
        ...
        if not guide_id or not submission_id:
            return api_response(
                message="Both guide_id and submission_id are required", status=400
            )
        ...
        if not guide or not submission:
            return api_response(message="Guide or submission not found", status=404)
        ...
        if result.success:
            ...
            return api_response(
                message=f"Processing failed: {result.error}", status=500
            )
...
def get_results():
    ...
    try:
        ...
        results = GradingResult.query.filter_by(user_id=current_user.id).paginate(
            page=page, per_page=per_page, error_out=False
        )
...
def get_cache_stats():
    ...
    try:
        ...
        try:
            from src.performance.query_cache import (
                get_cache_stats as get_query_cache_stats,
            )
        ...
        try:
            ...
            stats["optimization"] = (
                opt_manager.get_cache_stats()
                if hasattr(opt_manager, "get_cache_stats")
                else {}
            )
        except ImportError:
            pass
