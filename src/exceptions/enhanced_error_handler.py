...
class EnhancedErrorHandler:
    ...
    def get_recent_errors(
        ...
        """
        recent_errors = self.tracker.get_recent_errors(limit=limit * 2)  # Get more for filtering
    ...
    def _log_error(self, error: ApplicationError, context: Dict[str, Any]):
        ...
        if context:
            context_str = ", ".join([f"{k}={v}" for k, v in context.items() if k not in ['traceback']])
