...
class ErrorMapper(ABC):
    ...
    @abstractmethod
    def map_error(
        self, error: ApplicationError, context: Optional[Dict[str, Any]] = None
    ) -> UserMessage:
...
class UserFriendlyErrorMapper(ErrorMapper):
    ...
    def map_error(
        ...
        template = self._error_templates.get(
            error.error_code, self._error_templates[ErrorCode.INTERNAL_ERROR]
        )

        # Apply context-specific customizations
        customized_template = self._apply_context_customizations(
    ...
    def _apply_context_customizations(
        self, template: Dict[str, Any], error: ApplicationError, context: Dict[str, Any]
    ) -> Dict[str, Any]:
...
class ContextAwareErrorMapper(UserFriendlyErrorMapper):
    ...
    def _get_field_specific_message(
        self, field: str, details: Dict[str, Any]
    ) -> Optional[str]:
