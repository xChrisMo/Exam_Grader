...
class FlaskErrorIntegration:
    ...
    def _convert_http_exception_to_app_error(
    ...
    def _render_error_page(
        self, error: ApplicationError, response_data: Dict[str, Any]
    ) -> str:
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if request.path.startswith("/api/"):
                    status_code = enhanced_error_handler._get_http_status_code(
                        e.error_code
                    )
                    return jsonify(response_data), status_code
                else:
