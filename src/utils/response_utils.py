...
def standardize_response(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if isinstance(result, tuple) and len(result) == 2:
                ...
                if isinstance(data, dict) and "status" in data:
                    ...
                    response = APIResponse.success(
                        data=data, metadata=create_metadata(g.request_id)
                    )
            ...
            response = APIResponse.success(
                data=result, metadata=create_metadata(g.request_id)
            )
...
def error_response(
    ...
    response = ErrorResponse(
        message=message, error_code=error_code, field=field, details=details
    )
...
def validation_error_response(
    ...
    response = APIResponse(
        ...
        metadata=create_metadata(
            getattr(g, "request_id", None), validation_result.get_warning_messages()
        ),
    )
...
def create_error_response(
    ...
    """
    error_detail = ErrorDetail(
        code=error_code or "INTERNAL_ERROR", message=message, details=details or {}
    )

    error_response = ErrorResponse(
        status=ResponseStatus.ERROR, error=error_detail, metadata=create_metadata()
    )
