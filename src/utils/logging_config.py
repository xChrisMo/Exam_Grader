    ...
    def setup_logging(self, simplified: bool = True) -> None:
        ...
        if self.log_dir:
            file_handler = logging.FileHandler(
                self.log_dir / "exam_grader.log", mode="a", encoding="utf-8"
            )
...
================================================"""
    ...
    def get_environment_config() -> Dict[str, str]:
        ...
        return {
            ...
            "simplified_logging": os.getenv("SIMPLIFIED_LOGGING", "True").lower()
            == "true",
            "quiet_third_party": os.getenv("QUIET_THIRD_PARTY", "True").lower()
            == "true",
...
def setup_application_logging(
    ...
    simplified = (
        simplified if simplified is not None else env_config["simplified_logging"]
    )
    quiet_third_party = (
        quiet_third_party
        if quiet_third_party is not None
        else env_config["quiet_third_party"]
    )

    if (
        use_comprehensive
        and COMPREHENSIVE_LOGGING_AVAILABLE
        and setup_comprehensive_logging
    ):
        try:
            comprehensive_config = setup_comprehensive_logging(
                app_name="exam_grader", log_level=log_level, log_dir=log_dir or "logs"
            )
