...
class ResourceValidator:
    ...
    def _setup_default_requirements(self):
        ...
        self.add_requirement(ResourceRequirement(
            name="templates_dir",
            type="directory",
            path="templates",
    ...
    def validate_resources(self) -> Dict[str, Any]:
        ...
        logger.info(f"Resource validation completed: {valid_resources}/{total_resources} valid, {created_resources} created")
        
        return summary
    
    def _validate_single_resource(self, requirement: ResourceRequirement) -> ValidationResult:
        ...
        try:
            ...
            if not requirement.required:
                logger.info(f"Optional resource '{requirement.name}' not found, skipping")
                return ValidationResult(
                    valid=True,
                    resource_name=requirement.name,
                    resource_type=requirement.type,
    ...
    def get_validation_report(self) -> str:
        ...
        for result in self.validation_results:
            ...
            report_lines.append(f"  {status} {result.resource_name}: {result.message}{created_flag}")
        ...
        if failed_results:
            ...
            for result in failed_results:
                report_lines.append(f"  - {result.resource_name} ({result.resource_type}): {result.message}")
    ...
    def check_resource_permissions(self) -> Dict[str, Any]:
        ...
        for requirement in self.resource_requirements:
            ...
            if resource_path.exists():
                try:
                    ...
                    executable = os.access(resource_path, os.X_OK) if resource_path.is_dir() else False
                    ...
                    permission_results.append({
                        ...
                        'permissions_ok': readable and (writable if requirement.type != 'directory' else True)
                    })
        ...
        return {
            ...
            'permissions_ok': len([r for r in permission_results if r.get('permissions_ok', False)]),
    ...
    def cleanup_temporary_resources(self) -> Dict[str, Any]:
        ...
        if temp_dir.exists():
            try:
                ...
                cleanup_results['errors'].append(f"Error cleaning temp directory: {str(e)}")
        ...
        logger.info(f"Cleanup completed: {cleanup_results['files_removed']} files, {cleanup_results['directories_removed']} directories removed")
