...
from src.services.monitoring_dashboard import MonitoringAlert, AlertSeverity, monitoring_dashboard_service
...
class EnhancedAlertingSystem:
    ...
    def _setup_alert_thresholds(self):
        ...
        if not self._thresholds:
            self._thresholds = {
                ...
                AlertRule.HIGH_RESPONSE_TIME: AlertThreshold("response_time_ms", 5000, 10000),
                AlertRule.RESOURCE_EXHAUSTION: AlertThreshold("resource_usage", 0.80, 0.95),
                AlertRule.CACHE_MISS_RATE: AlertThreshold("cache_miss_rate", 0.70, 0.90),
            }
    
    def _setup_notification_channels(self):
        """Setup notification channels from configuration"""
        if self._error_config and self._error_config.get('notification', {}).get('enabled'):
            ...
            self._notification_config.channels = [
                AlertChannel(channel) for channel in channels if channel in [c.value for c in AlertChannel]
            ]
            
            # Set cooldown
            self._notification_config.cooldown_minutes = notification_config.get('cooldown_minutes', 15)
            
            logger.info(f"Configured notification channels: {[c.value for c in self._notification_config.channels]}")
        else:
            # Default to log only
            ...
            for service_name, service_health in health_status.get('services', {}).items():
                if service_health.get('status') == 'unhealthy':
                    ...
                    self._trigger_alert(
                        ...
                        AlertSeverity.HIGH,
                        {'service': service_name, 'error_count': service_health.get('error_count')}
                    )
            ...
            if threshold:
                ...
                if severity:
                    self._trigger_alert(
                        ...
                        severity,
                        {'response_time': avg_response_time, 'threshold': threshold.warning_value}
                    )
            ...
            if threshold:
                ...
                if severity:
                    self._trigger_alert(
                        ...
                        severity,
                        {'error_rate': error_rate, 'threshold': threshold.warning_value}
                    )
                            ...
                            if threshold:
                                ...
                                if severity:
                                    self._trigger_alert(
                                        ...
                                        severity,
                                        {'cache': cache_name, 'miss_rate': level_miss_rate}
                                    )
        
        except Exception as e:
            logger.error(f"Error checking processing alerts: {e}")
    
    def _trigger_alert(self, rule: AlertRule, title: str, message: str, 
                      severity: AlertSeverity, metadata: Dict[str, Any]):
        ...
        with self._lock:
            ...
            if rule_key in self._alert_states:
                ...
                if (alert_state.last_sent and 
                    current_time - alert_state.last_sent < timedelta(minutes=self._notification_config.cooldown_minutes)):
    ...
    def _send_alert_notification(self, title: str, message: str, 
                                severity: AlertSeverity, metadata: Dict[str, Any]):
    ...
    def _send_log_alert(self, title: str, message: str, severity: AlertSeverity, metadata: Dict[str, Any]):
        """Send alert to log"""
    ...
    def _send_email_alert(self, title: str, message: str, severity: AlertSeverity, metadata: Dict[str, Any]):
        """Send alert via email"""
        # This would require email configuration
        logger.info(f"Email alert would be sent: {title}")
    
    def _send_webhook_alert(self, title: str, message: str, severity: AlertSeverity, metadata: Dict[str, Any]):
        """Send alert via webhook"""
        # This would require webhook configuration
        logger.info(f"Webhook alert would be sent: {title}")
    
    def _send_console_alert(self, title: str, message: str, severity: AlertSeverity, metadata: Dict[str, Any]):
    ...
    def _process_escalations(self):
        ...
        with self._lock:
            for rule_key, alert_state in self._alert_states.items():
                if (not alert_state.escalated and 
                    current_time - alert_state.first_triggered > timedelta(minutes=self._notification_config.escalation_minutes)):
    ...
    def get_alert_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            escalated_alerts = sum(1 for state in self._alert_states.values() if state.escalated)
            
            return {
                'active_alerts': active_alerts,
                'escalated_alerts': escalated_alerts,
                'total_alert_rules': len(self._thresholds),
                'notification_channels': [c.value for c in self._notification_config.channels],
