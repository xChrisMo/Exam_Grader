...
Grade fairly and provide accurate scores based on answer quality. Use the full range from 0 to max_score for each question."""
    ...
    def _instant_fallback_grading(self, mappings: List[Dict]) -> Dict[str, Any]:
        ...
        for i, mapping in enumerate(mappings):
            ...
            score = self._calculate_fallback_score(answer, question, max_score)
    ...
    def _calculate_fallback_score(self, answer: str, question: str, max_score: float = 10.0) -> float:
        """Calculate a more accurate fallback score relative to max_score."""
        ...
        percentage_score = 50  # Start with neutral score
        ...
        if question_lower and len(question_lower) > 10:
            ...
            if overlap > 0:
                percentage_score += min(overlap * 3, 15)  # Max 15 bonus points

        # Ensure percentage is within bounds (0-100)
        percentage_score = max(0, min(100, percentage_score))
        
        # Convert to actual score relative to max_score
        actual_score = (percentage_score / 100.0) * max_score
