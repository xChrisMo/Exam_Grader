...
class ServiceMetrics:
    ...
    def add_custom_metric(self, key: str, value: Any) -> None:
        ...
        """
        if (
            key in self.custom_metrics
            and isinstance(self.custom_metrics[key], (int, float))
        ...
        with cls._lock:
            for name, service in cls._services.items():
                try:
                    ...
                    logger.info(
                        f"Service {name} initialization: {'success' if success else 'failed'}"
                    )
                except Exception as e:
                    results[name] = False
    ...
    def inject_dependencies(target_service: BaseService, **dependencies) -> None:
        ...
        for name, dependency in dependencies.items():
            if isinstance(dependency, BaseService):
                ...
                ServiceRegistry.add_dependency(
                    target_service.service_name, dependency.service_name
                )
                logger.debug(
                    f"Injected {dependency.service_name} into {target_service.service_name} as {name}"
                )
            else:
                setattr(target_service, name, dependency)
                logger.debug(
                    f"Injected {type(dependency).__name__} into {target_service.service_name} as {name}"
                )
    ...
    def auto_inject(target_service: BaseService) -> None:
        ...
        for dep_name in dependencies:
            ...
            if dep_service:
                ...
                logger.debug(
                    f"Auto-injected {dep_name} into {target_service.service_name}"
                )
