...
class ServiceRegistry:
            ...
            try:
                if hasattr(service, 'health_check'):
                    ...
                    if isinstance(is_healthy, bool):
                        ...
                        status = ServiceStatus(status_str) if status_str in [s.value for s in ServiceStatus] else ServiceStatus.UNKNOWN
                    else:
    ...
    def get_registry_status(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            for name, service_info in self.services.items():
                service_states[service_info.state.value] = service_states.get(service_info.state.value, 0) + 1
                health_statuses[service_info.health_status.value] = health_statuses.get(service_info.health_status.value, 0) + 1
    ...
    def _start_dependencies(self, service_name: str) -> bool:
        ...
        for dep_name in service_info.dependencies:
            if not self.start_service(dep_name):
                logger.error(f"Failed to start dependency '{dep_name}' for service '{service_name}'")
                return False
    ...
    def _start_service(self, name: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Starting service: {name} (attempt {service_info.initialization_attempts})")
