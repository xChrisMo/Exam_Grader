...
class EnhancedResultService:
    ...
    def enhance_result(self, result: Dict[str, Any], submission: Optional[Dict] = None,
                      guide: Optional[Dict] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            detailed_grades = self._parse_detailed_feedback(result.get('detailed_feedback'))
            
            # Enhance detailed grades with analytics
            if detailed_grades:
                enhanced['detailed_grades_enhanced'] = self._enhance_detailed_grades(detailed_grades)
                enhanced['grade_analytics'] = self._calculate_grade_analytics(detailed_grades)
            ...
            if detailed_grades:
                enhanced['question_analytics'] = self._calculate_question_analytics(detailed_grades)
            
            # Format processing metadata
            if result.get('processing_metadata'):
                enhanced['processing_metadata_formatted'] = self._format_processing_metadata(
                    result['processing_metadata']
                )
            
            # Calculate additional statistics
            enhanced['result_statistics'] = self._calculate_result_statistics(result, detailed_grades)
            
            # Add grading quality indicators
            enhanced['quality_indicators'] = self._assess_grading_quality(detailed_grades, result)
            
            logger.info(f"Enhanced result {result.get('id', 'unknown')} with additional analytics")
            return enhanced
    ...
    def _enhance_detailed_grades(self, detailed_grades: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, grade in enumerate(detailed_grades):
            ...
            enhanced_grade['score'] = float(grade.get('score', 0))
            enhanced_grade['max_score'] = float(grade.get('max_score', 10))
            ...
            enhanced_grade['performance_level'] = self._get_performance_level(enhanced_grade['percentage'])
            
            # Add score color coding
            enhanced_grade['score_color'] = self._get_score_color(enhanced_grade['percentage'])
            
            # Analyze feedback sentiment
            enhanced_grade['feedback_sentiment'] = self._analyze_feedback_sentiment(enhanced_grade['feedback'])
            
            # Add question and student answer if available
            enhanced_grade['question_text'] = grade.get('question_text', grade.get('question', 'Question text not available'))
            enhanced_grade['student_answer'] = grade.get('student_answer', grade.get('answer_text', 'Answer not available'))
            
            # Truncate long text for display
            enhanced_grade['question_text_short'] = self._truncate_text(enhanced_grade['question_text'], 100)
            enhanced_grade['student_answer_short'] = self._truncate_text(enhanced_grade['student_answer'], 150)
            enhanced_grade['feedback_short'] = self._truncate_text(enhanced_grade['feedback'], 100)
            
            enhanced_grades.append(enhanced_grade)
        
        return enhanced_grades

    def _calculate_grade_analytics(self, detailed_grades: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Calculate analytics across all grades."""
        if not detailed_grades:
            return {}
        
        scores = [float(grade.get('score', 0)) for grade in detailed_grades]
        max_scores = [float(grade.get('max_score', 10)) for grade in detailed_grades]
        percentages = [
        ...
        analytics = {
            ...
            'average_percentage': round(statistics.mean(percentages), 1) if percentages else 0,
            'median_percentage': round(statistics.median(percentages), 1) if percentages else 0,
            'highest_score': max(scores) if scores else 0,
            'lowest_score': min(scores) if scores else 0,
            'score_range': round(max(scores) - min(scores), 2) if len(scores) > 1 else 0,
        }
    ...
    def _analyze_confidence(self, confidence: float) -> Dict[str, Any]:
        ...
        if confidence >= self.confidence_thresholds['high']:
            ...
            level = 'very_low'
            interpretation = 'Very low confidence - results should be reviewed carefully'
            color = 'red'
    ...
    def _generate_performance_insights(self, detailed_grades: List[Dict[str, Any]], 
                                     overall_percentage: float) -> List[str]:
        ...
        if overall_percentage >= 90:
            insights.append("Excellent overall performance with strong understanding across topics")
        elif overall_percentage >= 80:
            insights.append("Good overall performance with solid grasp of most concepts")
        elif overall_percentage >= 70:
            insights.append("Fair performance with room for improvement in several areas")
        ...
        # Analyze consistency
        percentages = [
            (float(grade.get('score', 0)) / float(grade.get('max_score', 10)) * 100) 
            if float(grade.get('max_score', 10)) > 0 else 0
        ...
        if len(percentages) > 1:
            ...
            if std_dev < 10:
                ...
                insights.append("Highly variable performance - some topics well understood, others need work")
            else:
                insights.append("Moderate variation in performance across different topics")
        ...
        if percentages:
            ...
            if max_score_idx < len(detailed_grades):
                best_question = detailed_grades[max_score_idx].get('question_id', f'Q{max_score_idx + 1}')
                insights.append(f"Strongest performance on {best_question}")
            
            if min_score_idx < len(detailed_grades) and max(percentages) - min(percentages) > 20:
                worst_question = detailed_grades[min_score_idx].get('question_id', f'Q{min_score_idx + 1}')
                insights.append(f"Needs improvement on {worst_question}")
        
        return insights

    def _calculate_question_analytics(self, detailed_grades: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for grade in detailed_grades:
            ...
            max_score = float(grade.get('max_score', 10))
            ...
            question_data.append({
                ...
                'difficulty_assessment': self._assess_question_difficulty(percentage),
        ...
        return {
            ...
            'most_difficult': min(question_data, key=lambda x: x['percentage']) if question_data else None,
            'easiest': max(question_data, key=lambda x: x['percentage']) if question_data else None,
            'average_feedback_length': round(
                statistics.mean([q['feedback_length'] for q in question_data]), 1
            ) if question_data else 0
        }
    ...
    def _calculate_result_statistics(self, result: Dict[str, Any], 
                                   detailed_grades: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if detailed_grades:
            ...
            total_questions = len(detailed_grades)
            questions_passed = sum(1 for g in detailed_grades 
                                 if (float(g.get('score', 0)) / float(g.get('max_score', 10))) >= 0.6)
            
            stats.update({
                'total_questions': total_questions,
                'questions_passed': questions_passed,
                'pass_rate': round((questions_passed / total_questions) * 100, 1) if total_questions > 0 else 0,
                'questions_failed': total_questions - questions_passed
            })
        
        return stats

    def _assess_grading_quality(self, detailed_grades: List[Dict[str, Any]], 
        ...
        if detailed_grades:
            ...
            quality['feedback_completeness'] = round(
                statistics.mean(feedback_scores) * 25, 1
            ) if feedback_scores else 0
            
            # Assess scoring consistency (simplified)
            percentages = [
                (float(g.get('score', 0)) / float(g.get('max_score', 10)) * 100)
                if float(g.get('max_score', 10)) > 0 else 0
                for g in detailed_grades
            ]
            
            if len(percentages) > 1:
                consistency_cv = statistics.stdev(percentages) / statistics.mean(percentages) if statistics.mean(percentages) > 0 else 1
        ...
        if quality['has_detailed_feedback'] and quality['feedback_completeness'] > 50 and confidence > 0.8:
    ...
    def _analyze_feedback_sentiment(self, feedback: str) -> str:
        ...
        positive_words = ['good', 'excellent', 'correct', 'well', 'strong', 'clear', 'accurate']
        negative_words = ['poor', 'incorrect', 'missing', 'weak', 'unclear', 'incomplete', 'wrong']
