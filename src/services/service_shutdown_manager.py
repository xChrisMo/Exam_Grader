...
class ServiceShutdownManager:
    ...
    def register_service(
        ...
        with self._shutdown_lock:
            if self._shutdown_initiated:
                logger.warning(
                    f"Cannot register service {name} - shutdown already initiated"
                )
                return False
            ...
            logger.info(
                f"Registered service for shutdown: {name} (priority: {priority.name})"
            )
            return True
    ...
    def shutdown(self, timeout: float = 60.0) -> ShutdownResult:
        ...
        with self._shutdown_lock:
            ...
            try:
                ...
                logger.info(
                    f"Graceful shutdown completed in {result.total_duration:.2f}s"
                )
    ...
    def _shutdown_services(self, result: ShutdownResult, timeout: float):
        ...
        for priority in sorted(services_by_priority.keys(), key=lambda x: x.value):
            ...
            logger.info(
                f"Shutting down {len(services)} services with priority {priority.name}"
            )
    ...
    def _finalize_shutdown(self, result: ShutdownResult):
        ...
        logger.info(
            f"Shutdown summary: {result.services_shutdown} services shutdown, "
    ...
    def get_shutdown_stats(self) -> Dict[str, Any]:
        ...
        with self._shutdown_lock:
            return {
                ...
                "shutdown_result": (
                    self._shutdown_result.to_dict() if self._shutdown_result else None
                ),
