...
class ContentValidator:
    ...
    def __init__(self):
        ...
        self.english_indicators = [
            ...
            r'\bfor\b', r'\bwith\b', r'\bas\b', r'\bthis\b', r'\bhave\b',
            r'\bfrom\b', r'\bor\b', r'\bone\b', r'\bhad\b', r'\bby\b',
            r'\bword\b', r'\bbut\b', r'\bnot\b', r'\bwhat\b', r'\ball\b'
        ]
    ...
    def validate_content(self, content: str, file_info: Dict[str, Any] = None) -> ValidationResult:
        """
        ...
        logger.debug(f"Content validation completed: {result.quality.value} "
    ...
    def _basic_validation(self, content: str, result: ValidationResult) -> bool:
        ...
        if word_count < self.min_word_count:
            ...
            result.warnings.append(f"Content has only {word_count} words (minimum: {self.min_word_count})")
        
        if char_count < self.min_character_count:
            result.status = ValidationStatus.WARNING
            result.warnings.append(f"Content has only {char_count} characters (minimum: {self.min_character_count})")
    ...
    def _calculate_metrics(self, content: str) -> QualityMetrics:
        ...
        if metrics.sentence_count > 0:
            total_words_in_sentences = sum(len(sentence.split()) for sentence in sentences)
            metrics.average_sentence_length = total_words_in_sentences / metrics.sentence_count
    ...
    def _assess_content_length(self, result: ValidationResult):
        ...
        if metrics.word_count < 10:
            result.warnings.append("Very short content - may not be sufficient for analysis")
    ...
    def _assess_language_quality(self, content: str, result: ValidationResult):
        ...
        if language_confidence < self.min_language_confidence:
            result.warnings.append(f"Low English language confidence ({language_confidence:.2f})")
        ...
        if len(words) > 10:
            ...
            if repetition_ratio > self.max_repetition_ratio:
                result.warnings.append(f"High word repetition detected ({repetition_ratio:.2f})")
    ...
    def _assess_structure_quality(self, content: str, result: ValidationResult):
        ...
        if re.search(self.structure_patterns['paragraphs'], content):
            ...
            result.suggestions.append("Consider adding paragraph breaks for better readability")
        
        if re.search(self.structure_patterns['headers'], content, re.MULTILINE):
            structure_score += 15
        
        has_bullets = bool(re.search(self.structure_patterns['bullet_points'], content, re.MULTILINE))
        has_numbers = bool(re.search(self.structure_patterns['numbered_lists'], content, re.MULTILINE))
        ...
        if len(valid_sentences) > 0:
            sentence_score = min(25, (len(valid_sentences) / max(1, result.metrics.sentence_count)) * 25)
            structure_score += sentence_score
        
        # Check capitalization (25 points)
        capitalized_sentences = len(re.findall(self.structure_patterns['capitalized_sentences'], content))
    ...
    def _assess_coherence_quality(self, content: str, result: ValidationResult):
        ...
        if 10 <= avg_sentence_length <= 25:
            ...
            result.suggestions.append("Consider adjusting sentence length for better readability")
        ...
        if result.metrics.paragraph_count > 0 and result.metrics.word_count > 0:
            words_per_paragraph = result.metrics.word_count / result.metrics.paragraph_count
    ...
    def _assess_diversity_quality(self, content: str, result: ValidationResult):
        ...
        if diversity_score < 0.3:
            result.warnings.append("Low content diversity - consider varying vocabulary")
    ...
    def _detect_common_issues(self, content: str, result: ValidationResult):
        ...
        if re.search(r'(.)\1{4,}', content):
            result.warnings.append("Repeated characters detected - may indicate extraction errors")
        ...
        if words_without_punctuation > 20 and punctuation_marks < words_without_punctuation * 0.1:
            result.warnings.append("Low punctuation density - may indicate extraction issues")
    ...
    def _generate_suggestions(self, result: ValidationResult):
        ...
        if metrics.word_count < 50:
            result.suggestions.append("Consider extracting more content or checking extraction method")
        
        if metrics.average_sentence_length < 5:
            result.suggestions.append("Sentences appear very short - check for extraction fragmentation")
        elif metrics.average_sentence_length > 30:
            result.suggestions.append("Sentences appear very long - may need better sentence segmentation")
        
        if metrics.paragraph_count == 0:
            result.suggestions.append("No paragraph breaks detected - consider improving text structure")
        
        if metrics.language_confidence < 0.5:
            result.suggestions.append("Low English language confidence - verify content language or extraction quality")
        
        if metrics.diversity_score < 0.3:
            result.suggestions.append("Low vocabulary diversity - may indicate repetitive or poor quality content")
        
        if result.quality == ContentQuality.POOR:
            result.suggestions.append("Consider trying alternative extraction methods for better quality")
    
    def validate_batch(self, contents: List[Tuple[str, Dict[str, Any]]]) -> List[ValidationResult]:
    ...
    def get_quality_summary(self, results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        for result in results:
            quality_counts[result.quality.value] = quality_counts.get(result.quality.value, 0) + 1
