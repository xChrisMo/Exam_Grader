...
from src.services.processing_error_handler import FallbackStrategy, ErrorContext
...
class FallbackManager:
    ...
    def register_fallback(
        ...
        logger.info(f"Registered fallback method '{name}' for operation '{operation}' with priority {priority.value}")
    ...
    async def execute_with_fallback_async(
        ...
        **kwargs
        ...
        try:
            ...
            logger.warning(f"Primary async function failed for {operation}: {primary_error}")
            
            # Try fallback methods
            return await self._execute_fallbacks_async(operation, primary_error, *args, **kwargs)
        ...
        **kwargs
        ...
        for method in fallback_methods:
            ...
            if self._is_circuit_breaker_open(operation, method.name):
                logger.debug(f"Circuit breaker open for {operation}.{method.name}, skipping")
                continue
            
            try:
                logger.info(f"Trying fallback method '{method.name}' for operation '{operation}'")
                
                start_time = time.time()
                
                if method.timeout:
                    result = self._execute_with_timeout(method.function, method.timeout, *args, **kwargs)
                else:
                ...
                logger.info(f"Fallback method '{method.name}' succeeded for operation '{operation}'")
                ...
                logger.warning(f"Fallback method '{method.name}' failed: {fallback_error}")
    ...
    async def _execute_fallbacks_async(
        ...
        **kwargs
        ...
        for method in fallback_methods:
            ...
            try:
                logger.info(f"Trying async fallback method '{method.name}' for operation '{operation}'")
                
                start_time = time.time()
                
                if method.timeout:
                    if asyncio.iscoroutinefunction(method.function):
                        result = await asyncio.wait_for(method.function(*args, **kwargs), timeout=method.timeout)
                    else:
                        result = await asyncio.wait_for(
                            asyncio.get_event_loop().run_in_executor(None, method.function, *args, **kwargs),
                ...
                logger.info(f"Async fallback method '{method.name}' succeeded for operation '{operation}'")
                ...
                logger.warning(f"Async fallback method '{method.name}' failed: {fallback_error}")
    ...
    def _keyword_mapping(self, guide_text: str, submission_text: str, **kwargs) -> List[Dict]:
        """Keyword-based mapping fallback."""
        # Simple keyword matching
        import re
        
        questions = re.findall(r'(?:Question|Q)\s*\d+[:.]\s*([^?]+\?)', guide_text, re.IGNORECASE)
    ...
    def _simple_mapping(self, guide_text: str, submission_text: str, **kwargs) -> List[Dict]:
    ...
    def _rule_based_grading(self, submission_content: str, **kwargs) -> Dict[str, Any]:
        ...
        char_count = len(submission_content)
        ...
        if '.' in submission_content:
            base_score += 5
        if any(word in submission_content.lower() for word in ['because', 'therefore', 'however']):
            base_score += 5
    ...
    def _length_based_grading(self, submission_content: str, **kwargs) -> Dict[str, Any]:
    ...
    def enable_fallback_method(self, operation: str, method_name: str):
        ...
        for method in methods:
            if method.name == method_name:
                method.enabled = True
                logger.info(f"Enabled fallback method '{method_name}' for operation '{operation}'")
                return
        
        logger.warning(f"Fallback method '{method_name}' not found for operation '{operation}'")
    ...
    def disable_fallback_method(self, operation: str, method_name: str):
        ...
        for method in methods:
            if method.name == method_name:
                ...
                logger.info(f"Disabled fallback method '{method_name}' for operation '{operation}'")
                return
