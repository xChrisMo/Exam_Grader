...
class RealtimeMetricsCollector:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Real-time metrics collector initialized (interval: {self._collection_interval}s)"
        )
    ...
    def _setup_default_metrics(self):
        ...
        self.register_metric(
            "system.cpu.usage", MetricType.GAUGE, "CPU usage percentage", "%"
        )
        self.register_metric(
            "system.memory.usage", MetricType.GAUGE, "Memory usage percentage", "%"
        )
        self.register_metric(
            "system.disk.usage", MetricType.GAUGE, "Disk usage percentage", "%"
        )
        self.register_metric(
        ...
        self.register_metric(
            "app.requests.success_rate", MetricType.GAUGE, "Request success rate", "%"
        )
        self.register_metric(
            "app.response_time.avg", MetricType.GAUGE, "Average response time", "ms"
        )
        self.register_metric(
            "app.response_time.p95",
            MetricType.GAUGE,
        ...
        self.register_metric(
            "cache.size.bytes", MetricType.GAUGE, "Cache size in bytes", "bytes"
        )
        self.register_metric(
        ...
        # Processing metrics
        self.register_metric(
            "processing.queue.size", MetricType.GAUGE, "Processing queue size", "count"
        )
        self.register_metric(
    ...
    def _setup_default_collectors(self):
        ...
        self.register_collector(
            "service_health_metrics", self._collect_service_health_metrics
        )
    ...
    def register_collector(
        self, name: str, collector_func: Callable[[], Dict[str, float]]
    ):
    ...
    def get_metric_history(self, name: str, minutes: int = 60) -> List[Dict[str, Any]]:
        ...
        cutoff_time = datetime.now(timezone.utc) - timedelta(minutes=minutes)
        return [
            point.to_dict() for point in metric.points if point.timestamp >= cutoff_time
        ]
    ...
    def _collect_application_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            return {
                ...
                "app.requests.success_rate": perf_summary.get("success_rate", 0.0)
                * 100,
