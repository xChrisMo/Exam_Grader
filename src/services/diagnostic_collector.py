...
class DiagnosticCollector:
            ...
            for service_name in service_registry.list_services():
                ...
                if service_metrics:
                    metrics[service_name] = {
                        'response_times': [m.value for m in service_metrics if m.name == 'response_time'][-10:],
                        'last_response_time': service_metrics[-1].value if service_metrics else 0
                    }
    ...
    def collect_configuration_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            config_info['installed_packages'] = []  # Could use pkg_resources to get this
    ...
    def generate_recommendations(self, diagnostic_data: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if cpu_info.get('overall_percent', 0) > 80:
                recommendations.append("High CPU usage detected. Consider optimizing processing algorithms or scaling resources.")
            ...
            for mount_point, disk_data in disk_info.items():
                if disk_data.get('percent', 0) > 90:
                    recommendations.append(f"Disk space critical on {mount_point}. Clean up old files or increase storage capacity.")
            ...
            if overall_health.get('status') == 'unhealthy':
                recommendations.append("System health is unhealthy. Check service logs and restart failed services.")
            elif overall_health.get('status') == 'degraded':
                recommendations.append("System health is degraded. Monitor service performance and consider maintenance.")
            ...
            if total_errors > 100:
                recommendations.append("High error count detected. Review error logs and implement fixes for common issues.")
            
            # Performance recommendations
            process_info = diagnostic_data.get('process_info')
            if process_info and process_info.memory_mb > 1000:  # 1GB
                recommendations.append("High memory usage by application process. Consider memory optimization.")
    ...
    def export_diagnostic_report(self, output_file: str, report: DiagnosticReport = None) -> bool:
