    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "memory_usage_percent": (self._stats.total_size / self.max_memory)
                * 100,
    ...
    def _evict_entries(self, count: int) -> bool:
        ...
        if self.policy == CachePolicy.LRU:
            ...
            sorted_entries = sorted(
                self._entries.items(), key=lambda x: x[1].last_accessed
            )
            ...
            sorted_entries = sorted(
                self._entries.items(), key=lambda x: x[1].access_count
            )
            entries_to_evict = sorted_entries[:count]
            ...
            sorted_entries = sorted(
                self._entries.items(), key=lambda x: x[1].created_at
            )
            entries_to_evict = sorted_entries[:count]
...
class CacheManager:
    ...
    def __init__(self, cache_dir: str = "cache"):
        ...
        self._cleanup_interval = (
            self._cache_config.cleanup_interval if self._cache_config.enabled else 300
        )
        self._running = False
            ...
            if current_index > 0:  # Can promote
                ...
                if higher_level in self._levels:
                    if self._levels[higher_level].get(key) is None:
                        ...
                        logger.debug(
                            f"Promoted {key} from {current_level} to {higher_level}"
                        )
