...
from src.services.processing_error_handler import RetryConfig, ErrorCategory
...
class RetryManager:
    ...
    def _setup_configs_from_manager(self):
        ...
        try:
            services = ['ocr_processing', 'llm_processing', 'file_processing', 'mapping_service', 'grading_service']
            ...
            logger.error(f"Failed to load retry configurations from config manager: {e}")
    ...
    def retry_with_backoff(
        ...
        **kwargs
        ...
        for attempt_num in range(1, config.max_attempts + 1):
            ...
            try:
                logger.debug(f"Attempt {attempt_num}/{config.max_attempts} for operation: {operation}")
                ...
                # Update statistics
                self._update_retry_statistics(operation, attempt_num, True, time.time() - start_time)
                
                # Close circuit breaker on success
                self._close_circuit_breaker(operation)
                
                logger.info(f"Operation '{operation}' succeeded on attempt {attempt_num}")
                ...
                logger.warning(f"Attempt {attempt_num}/{config.max_attempts} failed for operation '{operation}': {e}")
                ...
                if attempt_num < config.max_attempts:
                    ...
                    logger.info(f"Retrying operation '{operation}' in {delay:.2f} seconds...")
                    time.sleep(delay)
                else:
        ...
        logger.error(f"Operation '{operation}' failed after {config.max_attempts} attempts")
    ...
    async def retry_with_backoff_async(
        ...
        **kwargs
        ...
        for attempt_num in range(1, config.max_attempts + 1):
            ...
            try:
                logger.debug(f"Async attempt {attempt_num}/{config.max_attempts} for operation: {operation}")
                ...
                logger.info(f"Async operation '{operation}' succeeded on attempt {attempt_num}")
                ...
                logger.warning(f"Async attempt {attempt_num}/{config.max_attempts} failed for operation '{operation}': {e}")
                
                if not self._is_retryable_error(e):
                    logger.info(f"Non-retryable error for async operation '{operation}': {e}")
                ...
                if attempt_num < config.max_attempts:
                    ...
                    logger.info(f"Retrying async operation '{operation}' in {delay:.2f} seconds...")
        ...
        logger.error(f"Async operation '{operation}' failed after {config.max_attempts} attempts")
    ...
    def _is_retryable_error(self, error: Exception) -> bool:
        ...
        non_retryable_patterns = [
            ...
            'invalid_request_error', 'permission', 'not found', '404',
            'bad request', '400', 'validation', 'malformed'
        ]
    ...
    def get_circuit_breaker_status(self) -> Dict[str, Any]:
        ...
        for operation, breaker in self.circuit_breakers.items():
            status[operation] = {
                ...
                'reset_time': breaker['reset_time'].isoformat() if breaker['reset_time'] else None,
    ...
    def with_retry(
        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                result = self.retry_with_backoff(operation, func, *args, strategy=strategy, **kwargs)
                if result.success:
                    return result.data
                else:
    ...
    def with_retry_async(
        ...
        def decorator(func):
            ...
            async def wrapper(*args, **kwargs):
                result = await self.retry_with_backoff_async(operation, func, *args, strategy=strategy, **kwargs)
                if result.success:
                    return result.data
                else:
                    raise Exception(result.final_error)
