...
from src.services.enhanced_processing_error_system import enhanced_error_system
...
class CoreService(BaseService):
    ...
    def _initialize_engines(self):
        ...
        with log_operation(LogCategory.SYSTEM, "core_service", "initialize_engines") as op:
            ...
            for service_name, init_func in services_to_initialize:
                ...
                try:
                    enhanced_logging_service.log_info(
                        LogCategory.SYSTEM, "core_service", f"initialize_{service_name}",
                    ...
                    enhanced_logging_service.log_error(
                        LogCategory.ERROR, "core_service", f"initialize_{service_name}",
                        error_msg, metadata={'service_name': service_name}
                    )
            ...
            health_monitor.register_service("core_service", self.health_check)
            
            # Determine overall initialization success
            successful_services = sum(1 for status in self._initialization_status.values() if status['success'])
            ...
            elif successful_services >= total_services * 0.5:  # At least 50% services working
    ...
    def _initialize_mapping_service(self) -> bool:
        ...
        try:
            ...
            if self._llm_engine is None:
                logger.warning("Mapping service requires LLM service - initializing without LLM dependency")
                self._mapping_service = ConsolidatedMappingService(llm_service=None)
            else:
                self._mapping_service = ConsolidatedMappingService(llm_service=self._llm_engine)
    ...
    def _initialize_grading_service(self) -> bool:
        ...
        try:
            ...
            if missing_deps:
                logger.warning(f"Grading service missing dependencies: {', '.join(missing_deps)} - initializing with available services")
    ...
    def _register_service(self, name: str, service):
        ...
        try:
            ...
            enhanced_logging_service.log_info(
                LogCategory.SYSTEM, "core_service", "register_service",
        ...
        try:
            if hasattr(service, 'health_check'):
                health_monitor.register_service(f"core_service_{name}", service.health_check)
            elif hasattr(service, 'is_available'):
                health_monitor.register_service(f"core_service_{name}", service.is_available)
        except Exception as e:
            enhanced_logging_service.log_warning(
                LogCategory.SYSTEM, "core_service", "register_service",
        ...
        try:
            if hasattr(service, 'health_check'):
                ...
                self._service_health[name]['status'] = ServiceStatus.HEALTHY if is_healthy else ServiceStatus.UNHEALTHY
            elif hasattr(service, 'is_available'):
                is_available = service.is_available()
                self._service_health[name]['status'] = ServiceStatus.HEALTHY if is_available else ServiceStatus.DEGRADED
            else:
            ...
            enhanced_logging_service.log_error(
                LogCategory.ERROR, "core_service", "register_service",
        ...
        for name, service in self._service_registry.items():
            try:
                if hasattr(service, 'health_check'):
                    ...
                    is_available = service.is_available()
                    status = ServiceStatus.HEALTHY if is_available else ServiceStatus.DEGRADED
                else:
    ...
    async def process_submission(self, request: ProcessingRequest) -> ProcessingResponse:
        """
        ...
        try:
            ...
            try:
                ...
                if not hasattr(guide, 'id') or guide.id is None:
                    ...
                    return ProcessingResponse(
                        success=False,
                        error="Invalid guide object - missing ID"
                    )
                ...
                if not hasattr(submission, 'id') or submission.id is None:
                    ...
                    return ProcessingResponse(
                        success=False,
                        error="Invalid submission object - missing ID"
                    )
                ...
                logger.info(f"Successfully loaded and validated guide {guide.id} and submission {submission.id}")
            ...
            if not submission_text:
                return ProcessingResponse(
                    success=False,
                    error="Failed to extract text from submission"
                )
            ...
            result_id = await self._save_results(
                request, grading_result, mappings
            )
    ...
    async def _extract_text(self, submission: Submission) -> Optional[str]:
        ...
        if submission.content_text:
            logger.info(f"Using pre-extracted text for submission {submission.id} ({len(submission.content_text)} chars)")
        ...
        if self._ocr_engine and submission.file_path:
            ...
            try:
                ...
                if extracted_text and extracted_text.get('success'):
                    ...
                    logger.info(f"OCR extraction successful for {submission.id} ({len(text_content)} chars)")
                    return text_content
                else:
            ...
            if result.success and result.data and result.data.get('success'):
                ...
                logger.info(f"OCR extraction successful for {submission.id} ({len(extracted_text)} chars)")
                return extracted_text
            else:
                logger.warning(f"OCR extraction failed for {submission.id}: {result.error_message}")
        
        logger.warning(f"No text content available for submission {submission.id}")
        return None
    
    async def _map_answers(self, guide: MarkingGuide, submission_text: str) -> Optional[List[Dict]]:
        ...
        try:
            ...
            if hasattr(self, '_mapping_service') and self._mapping_service:
                try:
                    mapping_result, error = self._mapping_service.map_submission_to_guide(
                        guide_content, submission_text, 5  # Limit to 5 for speed
                    )
                    
                    if mapping_result and mapping_result.get('mappings'):
                        logger.info(f"Fallback mapping service returned {len(mapping_result['mappings'])} mappings")
    ...
    async def _grade_submission(self, guide: MarkingGuide, mappings: List[Dict]) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(f"Starting ultra-fast grading for {len(mappings)} mapped answers")
    ...
    def _validate_request(self, request: ProcessingRequest) -> bool:
        ...
        return (
            request.guide_id and 
            request.submission_id and 
            request.user_id
        )
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.warning("CoreService initialization completed with some failures")
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            is_healthy = healthy_services >= total_services * 0.5 if total_services > 0 else False
    ...
    def handle_missing_service_dependency(self, service_name: str, dependency_name: str) -> bool:
    ...
    def get_available_services(self) -> List[str]:
        ...
        return [
            name for name, status in self._initialization_status.items()
            if status['success']
        ]
    
    def is_service_available(self, service_name: str) -> bool:
        """Check if a specific service is available."""
        return (service_name in self._initialization_status and 
