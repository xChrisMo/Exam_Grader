...
class FileLogHandler(LogHandler):
    ...
    def __init__(self, name: str, log_file: str, max_size: int = 10 * 1024 * 1024,
                 backup_count: int = 5, format_json: bool = True):
        super().__init__(name)
    ...
    def _process_entry(self, entry: StructuredLogEntry) -> bool:
        ...
        try:
            with self._lock:
                ...
                error_logs = (self.metrics.logs_by_level.get('ERROR', 0) + 
...
class EnhancedLoggingService:
            ...
            for dest_name, dest_config in destinations.items():
                ...
                if dest_config.path:  # File-based destination
                    handler = FileLogHandler(
                        ...
                        format_json=self._logging_config.structured_logging.get('enabled', True)
                    )
    ...
    def log(self, level: LogLevel, category: LogCategory, service: str,
            operation: str, message: str, user_id: Optional[str] = None,
    ...
    def log_info(self, category: LogCategory, service: str, operation: str,
                 message: str, **kwargs):
        """Log info level message"""
        self.log(LogLevel.INFO, category, service, operation, message, **kwargs)
    
    def log_warning(self, category: LogCategory, service: str, operation: str,
                    message: str, **kwargs):
        """Log warning level message"""
        self.log(LogLevel.WARNING, category, service, operation, message, **kwargs)
    
    def log_error(self, category: LogCategory, service: str, operation: str,
                  message: str, **kwargs):
    ...
    def log_performance(self, service: str, operation: str, duration_ms: float,
        ...
        self.log(level, LogCategory.PERFORMANCE, service, operation, message,
...
class OperationLogger:
    ...
    def __init__(self, logging_service: EnhancedLoggingService, 
                 category: LogCategory, service: str, operation: str,
    ...
    def __enter__(self):
        ...
        self.logging_service.log_info(
            self.category, self.service, self.operation,
        ...
        if self.start_time is not None:
            ...
            if self.success:
                self.logging_service.log_performance(
                    self.service, self.operation, duration_ms,
                    success=True, user_id=self.user_id,
                    request_id=self.request_id, metadata=self.metadata
                )
            else:
...
def log_operation(category: LogCategory, service: str, operation: str,
                 user_id: Optional[str] = None, request_id: Optional[str] = None,
                 metadata: Optional[Dict[str, Any]] = None):
    """Context manager for logging operations"""
    return OperationLogger(enhanced_logging_service, category, service, operation,
