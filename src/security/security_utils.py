...
class SecurityUtils:
            ...
            for pattern, secret_type in secret_patterns:
                ...
                for match in matches:
                    ...
                    if any(placeholder in value.lower() for placeholder in 
                          ['your_', 'example', 'placeholder', 'change_me', 'xxx']):
                        continue
    ...
    def generate_security_report() -> Dict[str, any]:
        ...
        for file_path in key_files:
            if file_path.exists():
                ...
                if findings:
                    ...
                    logger.warning(f"Found {len(findings)} potential secrets in {file_path}")
        ...
        if report['environment_audit']['issues']:
            report['recommendations'].append("Address environment variable security issues")
        
        if files_with_secrets > 0:
            report['recommendations'].append("Review and secure hardcoded secrets in files")
