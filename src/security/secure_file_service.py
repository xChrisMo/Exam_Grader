...
class SecureFileStorage:
    ...
    def store_file(
        self, file_content: bytes, filename: str, file_info: Dict[str, Any]
    ...
    ) -> Tuple[bool, Optional[str], Optional[bytes], Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            for ext in cls.SUSPICIOUS_EXTENSIONS:
                if ext in file_content:
                    threats.append(
                        f"Suspicious file extension in content: {ext.decode('utf-8', errors='ignore')}"
                    )
            ...
            if not is_clean:
                logger.warning(
                    f"Malware scan detected threats in {filename}: {threats}"
                )
...
class SecureFileService:
    ...
    def __init__(self, storage_path: str, enable_malware_scan: bool = True):
        ...
        logger.info(
            f"Secure file service initialized (malware scan: {enable_malware_scan})"
        )
    ...
    def process_upload(
        ...
        try:
            ...
            if self.enable_malware_scan:
                is_clean, threats = self.malware_scanner.scan_file(
                ...
                if not is_clean:
                    ...
                    logger.warning(
                        f"File {filename} quarantined due to threats: {threats}"
                    )

                    # Store in quarantine but don't fail the upload
                    success, store_error, stored_path = self.storage.store_file(
                        file_content, filename, file_info
                    )
                    if success:
                        file_info["stored_path"] = str(stored_path)
                        return (
                            True,
                            f"File quarantined due to security concerns: {', '.join(threats)}",
                            file_info,
                        )
                    else:
    ...
    def validate_file_access(
        self, stored_filename: str, user_id: Optional[str] = None
    ) -> bool:
        ...
        try:
            # Get file metadata
            success, error, content, metadata = self.storage.retrieve_file(
                stored_filename
            )
            if not success:
                return False
            ...
            logger.error(
                f"Error validating file access for {stored_filename}: {str(e)}"
            )
            return False
