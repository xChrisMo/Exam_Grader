...
try:
    ...
    ocr_service_instance = None  # Allow app to run without OCR
    ...
    def extract_text_from_pdf(file_path: str) -> str:
        ...
        try:
            ...
            if not text.strip() or len(text.strip()) < 10:
                logger.warning("PDF text extraction yielded insufficient text, OCR may be needed")
    ...
    def extract_text_from_image(file_path: str) -> str:
        ...
        try:
            ...
            if is_pdf:
                ...
                for page_num, page in enumerate(doc, 1):
                    try:
                        ...
                        try:
                            ...
                            logger.debug(f"Cleaned up temporary image for page {page_num}")
                        except Exception as e:
                            logger.warning(f"Failed to clean up temporary image {temp_img_path}: {str(e)}")
                ...
                logger.info(f"Successfully extracted {len(text)} characters from PDF using OCR")
                return text
            else:
                ...
                if ocr_service_instance:
                    ...
                    raise OCRServiceError("OCR service not available")
...
def parse_student_submission(file_path: str) -> Tuple[Dict[str, str], Optional[str], Optional[str]]:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Direct text extraction failed: {str(e)}. Attempting OCR fallback.")
            raw_text = None
        ...
        if (not raw_text or not raw_text.strip()) and not ocr_used and file_type != 'text/plain':
            try:
                ...
                raw_text = DocumentParser.extract_text_from_image(file_path)
                logger.info(f"OCR fallback successful, extracted {len(raw_text)} characters")
        ...
        logger.error(f"Error extracting text from submission: {str(e)}")
        return {}, None, str(e)