...
class DatabaseUtils:
    ...
    def get_database_stats() -> Dict[str, Any]:
        ...
        try:
            from .models import GradingResult, MarkingGuide, Submission, User
    ...
    def get_active_marking_guides(user_id: str) -> List[Any]:
        ...
        try:
            ...
            return (
                MarkingGuide.query.filter_by(user_id=user_id, is_active=True)
    ...
    def get_submissions_by_status(user_id: str, status: str) -> List[Any]:
        ...
        try:
            ...
            return (
                Submission.query.filter_by(user_id=user_id, processing_status=status)
                .order_by(Submission.created_at.desc())
                .all()
            )
        except Exception as e:
            logger.error(
                f"Error getting submissions for user {user_id}, status {status}: {str(e)}"
            )
            return []

    @staticmethod
    @monitor_performance(slow_threshold=2.0)
    def bulk_update_submission_status(
        submission_ids: List[str], new_status: str
    ) -> bool:
        """Bulk update submission status for better performance."""
        try:
            from .models import Submission, db

            updated = Submission.query.filter(Submission.id.in_(submission_ids)).update(
                {Submission.processing_status: new_status}, synchronize_session=False
            )
