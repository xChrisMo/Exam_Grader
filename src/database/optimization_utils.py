...
class DatabaseOptimizer:
        ...
        for table_name, expected in expected_indexes.items():
            try:
                ...
                existing_index_names = [
                    idx["name"] for idx in table_indexes if idx["name"]
                ]
                ...
                logger.warning(
                    f"Could not inspect indexes for table {table_name}: {str(e)}"
                )

        return {"existing": existing_indexes, "missing": missing_indexes}
    ...
    def validate_foreign_keys(self) -> Dict[str, List[str]]:
        ...
        for table_name, expected in expected_foreign_keys.items():
            try:
                ...
                logger.warning(
                    f"Could not inspect foreign keys for table {table_name}: {str(e)}"
                )

        return {"existing": existing_fks, "missing": missing_fks}
    ...
    def validate_constraints(self) -> Dict[str, List[str]]:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                missing_triggers = [
                    t for t in expected_triggers if t not in existing_triggers
                ]

                return {"existing": existing_triggers, "missing": missing_triggers}
    ...
    def validate_views(self) -> Dict[str, List[str]]:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                return {"existing": existing_views, "missing": missing_views}
    ...
    def optimize_database(self) -> Dict[str, any]:
        ...
        if result["success"]:
            ...
            logger.warning(
                "Database optimization completed with issues. Check the report for details."
            )
