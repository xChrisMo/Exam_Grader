...
try:
    from .models import TimestampMixin, db, get_uuid_column

    DB_AVAILABLE = db is not None
except ImportError:
    try:
        from src.database.models import TimestampMixin, db, get_uuid_column
...
class ProgressUpdate(BaseModel, TimestampMixin):
    ...
    session_id = Column(
        String(36),
        ForeignKey("progress_sessions.session_id"),
        nullable=False,
        index=True,
    )
...
class ProgressMetrics(BaseModel, TimestampMixin):
    ...
    )  # throughput, latency, error_rate, etc.
    metric_value = Column(Float, nullable=False)
    metric_unit = Column(
        String(20), nullable=True
    )  # seconds, requests/sec, percentage, etc.
