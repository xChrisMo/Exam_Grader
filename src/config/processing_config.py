...
class CacheConfig:
    ...
    def get_size_limit(self, cache_type: str) -> CacheSizeConfig:
        ...
        return self.size_limits.get(
            cache_type, self.size_limits.get("default", CacheSizeConfig())
        )
...
class ProcessingConfigManager:
    ...
    def _load_configuration(self):
        ...
        if os.path.exists(self.config_path):
            ...
            logger.warning(
                f"Processing configuration file not found: {self.config_path}"
            )
            self._config_data = {}
    ...
    def _apply_environment_overrides(self):
        ...
        if timeout_env:
            try:
                ...
                logger.info(
                    f"Applied timeout override: PROCESSING_DEFAULT_TIMEOUT={timeout_env}"
                )
            except ValueError as e:
        ...
        if cache_enabled:
            ...
            self._config_data["cache_configuration"]["enabled"] = (
                cache_enabled.lower() == "true"
            )
            logger.info(
                f"Applied cache override: PROCESSING_CACHE_ENABLED={cache_enabled}"
            )
        ...
        if health_enabled:
            ...
            self._config_data["health_check_configuration"]["enabled"] = (
                health_enabled.lower() == "true"
            )
            logger.info(
                f"Applied health check override: PROCESSING_HEALTH_CHECKS_ENABLED={health_enabled}"
            )
    ...
    def get_health_check_config(self) -> HealthCheckConfig:
        ...
        thresholds = HealthCheckThresholds(
            response_time_warning_ms=thresholds_data.get(
                "response_time_warning_ms", 5000
            ),
            response_time_critical_ms=thresholds_data.get(
        ...
        return HealthCheckConfig(
            ...
            consecutive_failures_threshold=health_data.get(
    ...
    def is_health_checks_enabled(self) -> bool:
        ...
        return self._config_data.get("health_check_configuration", {}).get(
            "enabled", True
        )
    ...
    def is_async_processing_enabled(self) -> bool:
        ...
        return (
            self._config_data.get("performance_optimization", {})
            .get("async_processing", {})
            .get("enabled", True)
        )
    ...
    def validate_configuration(self) -> List[str]:
        ...
        for service, policy in retry_policies.items():
            ...
            if policy.get("max_delay", 0) <= policy.get("base_delay", 1):
                warnings.append(
                    f"Invalid max_delay for {service}: must be > base_delay"
                )
        ...
        if cache_config.get("enabled", True):
            ...
            for cache_type, limits in size_limits.items():
                if limits.get("max_size_mb", 0) <= 0:
                    warnings.append(
                        f"Invalid max_size_mb for {cache_type}: must be > 0"
                    )
                if limits.get("max_entries", 0) <= 0:
                    warnings.append(
                        f"Invalid max_entries for {cache_type}: must be > 0"
                    )
        ...
        if health_config.get("enabled", True):
            ...
            for threshold_name, value in thresholds.items():
                if "rate" in threshold_name and not (0 <= value <= 1):
                    warnings.append(
                        f"Invalid {threshold_name}: must be between 0 and 1"
                    )
