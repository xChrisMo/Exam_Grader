...
class APIResponse:
    ...
    def success(cls, data: Any = None, message: str = None, **kwargs) -> "APIResponse":
        ...
        return cls(status=ResponseStatus.SUCCESS, data=data, message=message, **kwargs)

    @classmethod
    def error(
        cls, message: str, errors: List[ErrorDetail] = None, **kwargs
    ) -> "APIResponse":
        """Create an error response."""
        return cls(
            status=ResponseStatus.ERROR, message=message, errors=errors or [], **kwargs
        )
    ...
    def loading(
        ...
        **kwargs,
        ...
        return cls(status=ResponseStatus.LOADING, data=data, message=message, **kwargs)
...
class ErrorResponse(APIResponse):
    ...
    def not_found(cls, resource: str = "Resource") -> "ErrorResponse":
        ...
        return cls(message=f"{resource} not found", error_code=ErrorCode.NOT_FOUND)

    @classmethod
    def unauthorized(cls, message: str = "Authentication required") -> "ErrorResponse":
        """Create an unauthorized error response."""
        return cls(message=message, error_code=ErrorCode.AUTHENTICATION_ERROR)

    @classmethod
    def forbidden(cls, message: str = "Access denied") -> "ErrorResponse":
        """Create a forbidden error response."""
        return cls(message=message, error_code=ErrorCode.AUTHORIZATION_ERROR)
    ...
    def processing_error(
        cls, message: str = "Processing failed", details: Dict[str, Any] = None
    ) -> "ErrorResponse":
        """Create a processing error response."""
        return cls(
            message=message, error_code=ErrorCode.PROCESSING_ERROR, details=details
        )
...
def create_error_response(
    message: str, error_code: ErrorCode = ErrorCode.INTERNAL_ERROR
) -> Dict[str, Any]:
    """Create an error response dictionary."""
    return ErrorResponse(message=message, error_code=error_code).to_dict()

def create_loading_response(
    operation_id: str, message: str = "Processing...", progress: Dict[str, Any] = None
) -> Dict[str, Any]:
    """Create a loading response dictionary."""
    return APIResponse.loading(
        operation_id=operation_id, message=message, progress=progress
    ).to_dict()

def create_paginated_response(
    items: List[Any], page: int, per_page: int, total_items: int, message: str = None
) -> Dict[str, Any]:
