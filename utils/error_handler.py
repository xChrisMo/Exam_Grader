...
class ErrorHandler:
    ...
    def log_error(
        ...
        error_info = {
            ...
            "error_code": (
                error_code.value
                if error_code and hasattr(error_code, "value")
                else None
            ),
            "error_id": error_id,
            "context": context,
        ...
        if hasattr(error, "severity"):
            ...
            if severity in ["critical", "high"]:
                logger.error(
                    f"[{severity.upper()}] Error in {context}: {error_message}"
                )
...
class ProgressTracker:
    ...
    def complete_operation(
        ...
        if operation_id in self.operations:
            ...
            logger.info(
                f"Operation {operation_id} {'completed' if success else 'failed'}: {message}"
            )
...
def handle_error(
    ...
    if ENHANCED_ERROR_HANDLING_AVAILABLE and enhanced_error_handler:
        try:
            ...
            logger.warning(
                f"Enhanced error handling failed, falling back to legacy: {e}"
            )
