...
def is_guide_in_use(guide_id) -> bool:
    ...
    try:
        ...
        if active_submissions > 0:
            logger.warning(
                f"Guide {guide_id} is currently in use by {active_submissions} active submissions"
            )
            return True
        ...
        if recent_results > 0:
            logger.warning(
                f"Guide {guide_id} has recent processing activity ({recent_results} results in last 5 minutes)"
            )
            return True
