...
class LoadingManager:
    ...
    def start_operation(self, operation_id: str, operation_name: str, 
                       total_steps: int = 100, initial_message: str = "Starting...") -> ProgressInfo:
        """
    ...
    def update_progress(self, operation_id: str, current_step: Optional[int] = None,
                       message: Optional[str] = None, state: Optional[LoadingState] = None,
                       increment: bool = False) -> Optional[ProgressInfo]:
        """
        ...
        with self._lock:
            ...
            if progress.total_steps > 0:
                progress.progress_percent = min(100.0, (progress.current_step / progress.total_steps) * 100)
            ...
            logger.debug(f"Updated operation {operation_id}: {progress.progress_percent:.1f}% - {message}")
            return progress
    
    def add_sub_operation(self, operation_id: str, sub_operation: Dict[str, Any]) -> bool:
        """
    ...
    def complete_operation(self, operation_id: str, final_message: str = "Completed",
                          success: bool = True) -> Optional[ProgressInfo]:
        """
        ...
        with self._lock:
            ...
            logger.info(f"Completed operation {operation_id}: {final_message} in {progress.elapsed_time:.1f}s")
            return progress
    
    def fail_operation(self, operation_id: str, error_message: str) -> Optional[ProgressInfo]:
        """
    ...
    def get_active_operations(self) -> Dict[str, ProgressInfo]:
        ...
        with self._lock:
            return {
                op_id: progress for op_id, progress in self._operations.items()
                if progress.state not in [LoadingState.COMPLETE, LoadingState.ERROR, LoadingState.CANCELLED]
            }
    ...
    def cleanup_old_operations(self, max_age_seconds: int = 3600) -> int:
        ...
        with self._lock:
            ...
            for op_id, progress in self._operations.items():
                if progress.state in [LoadingState.COMPLETE, LoadingState.ERROR, LoadingState.CANCELLED]:
...
def create_loading_response(operation_id: str, message: str = "Processing...",
