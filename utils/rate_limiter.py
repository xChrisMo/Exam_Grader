...
class RateLimiter:
    ...
    def is_rate_limited(self, identifier: str, limit: int, window: int) -> bool:
        ...
        while (
            self.requests[identifier] and self.requests[identifier][0] <= now - window
        ):
...
class EnhancedRateLimiter(RateLimiter):
    ...
    def is_allowed(
        self, rule_name: str, max_requests: int = 100, window_seconds: int = 3600
    ):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if rate_limiter.is_rate_limited(identifier, limit, window):
                ...
                if request.is_json or request.path.startswith("/api/"):
                    ...
                    flash(
                        f"Too many requests. Please wait before trying again. Limit: {limit} per {per}",
                        "warning",
                    )
...
def get_rate_limit_status(identifier: Optional[str] = None) -> Dict:
    ...
    remaining = rate_limiter.get_remaining_requests(
        identifier, default_limit, default_window
    )
